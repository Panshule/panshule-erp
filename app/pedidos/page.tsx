...
export default function Pedidos() {
  ...
  const cargarDatos = async () => {
    // 1. Cargar todos los pedidos (para la lista)
    ...
    // 2. Calcular Totales (Finanzas de Jule)
    const hoy = new Date();
  ...
  return (
    <main className="min-h-screen p-6 bg-panshule-base flex flex-col items-center font-sans">
      <div className="w-full max-w-md">
        ...
        {/* Historial (Lista de abajo) */}
        <h2 className="text-2xl font-bold text-panshule-dark mb-4">Historial de Ventas</h2>
        <div className="space-y-4">
          {pedidos.length === 0 ? (
            <p className="text-center text-panshule-crust italic mt-10">Todavía no registraste ventas.</p>
          ) : (
            pedidos.map((p) => (
